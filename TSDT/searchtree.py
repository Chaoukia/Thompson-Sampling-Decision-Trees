...
#            self.sigma = np.sqrt(self.sigma)
    ...
    def set_mu_sigma(self, mu, sigma):
        ...
        Parameters
        -----------------------
        mu    : Float, mean of the prior.
        sigma : Float, standard deviation of the prior.
    ...
    def expand(self, leaf, thresh):
        ...
        for child in chain.from_iterable(self.children.values()):
            ...
            if len(params) == 2:
                mu, sigma = approximate_max_normal_2(params)
                self.set_mu_sigma(mu, sigma)
                params.pop()
                params[0] = (mu, sigma)
...
#        return norm.rvs(loc=self.mu, scale=self.sigma) - self.lambd*self.depth
...
class SearchTree:
    ...
    def select(self, thresh_tree, thresh_mu, thresh_sigma):
        """
        Description
        -----------------------
...
#            if (node.mu >= thresh_mu) and (node.sigma < thresh_sigma):
#                node.tree.update_value()
#                if node.tree.value >= -thresh_tree:
#                    return node
    ...
    def expand(self, searchtree_node, thresh_leaf):
        """
        Description
        -----------------------
        Expand a SearchNode.
        ...
        searchtree_node.expand(leaf, thresh_leaf)
    ...
    def backpropagate(self, searchtree_node):
        ...
        while parent is not None:
            # Update the poesterior of the terminal child.
            parent.children[0.5][0].update_mu_sigma()
            params = []
...
#            start_time = time()
            searchtree_node = self.select(thresh_tree, thresh_mu, thresh_sigma)
#            print('Selection:', time() - start_time)
#            if (searchtree_node.mu >= thresh_mu) and (searchtree_node.sigma < thresh_sigma):
#                # Backpropagation here is important if we still want to use max_value inference.
...
#                start_time = time()
                self.expand(searchtree_node, thresh_leaf)
